...
>>>>>>> e34982d0e84899a2a83678c7180eaf472fcf0e67
...
class Main extends Component {
  ...
  render() {
    ...
    if (foodTable)
      todayFoodTableCand = foodTable["foodData"].filter((elem) => {
        return (
          elem["date"] === `${year}${this.addZero(month)}${this.addZero(day)}`
        );
      });
    ...
    if (todayFoodTableCand && todayFoodTableCand.length > 0) {
      todayFoodTable = todayFoodTableCand.reduce((previous, current) => {
        ...
        return previous["brst"].length >= current["brst"].length
          ? previous
          : current;
      });
    }
